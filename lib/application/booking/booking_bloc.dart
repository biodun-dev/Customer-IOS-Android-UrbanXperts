  ...
  BookingBloc() : super(const BookingState()) {
    ...
    on<FetchPayments>((event, emit) async {
      ...
      res.fold((l) {
        ...
        emit(state.copyWith(
            list: list,
            selectPayment: event.payLater ? -1 : l.data?[index].id ?? -1,
    ...
    on<FetchWebView>((event, emit) async {
      final payment = state.list?.firstWhere(
          (element) => element.id == state.selectPayment,
          orElse: () => PaymentData());
      final res = await paymentsRepository.paymentBookWebView(
    ...
    on<SelectMaster>((event, emit) {
      ...
      final extra =
          service.firstWhere((e) => e.id == event.serviceId).selectExtras;
