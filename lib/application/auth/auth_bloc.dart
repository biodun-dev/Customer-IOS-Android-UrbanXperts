...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(this._authRepository) : super(const AuthState()) {
    ...
    on<SetVerificationId>((event, emit) {
      emit(state.copyWith(
    ...
    on<SwitchScreen>((event, emit) {
      forgetPassword = (event.type == AuthType.forgetPassword);
    ...
    on<CheckPhone>((event, emit) async {
      ...
      res.fold((l) {
        if (state.screenType == AuthType.forgetPassword && l) {
          emit(state.copyWith(isLoading: false));
          event.onSuccess?.call();
          return;
        }
        ...
        if (!l && state.screenType == AuthType.forgetPassword) {
          emit(state.copyWith(isLoading: false));
          AppHelper.errorSnackBar(
              context: event.context,
